 ...
 * \returns A `cli_args` object holding the parsed query file path and HIBF index path.
 * \details
 *
 * Sets up a sharg::parser exposing two required options: `-q`/`--query`, the query
 * file to align, and `-i`/`--index`, the HIBF index file to read (see `load()`).
 *
 * If parsing fails (e.g. an option is missing or the argument is invalid), the error
 * is printed to stderr and a default-constructed `cli_args` (empty `query_path` and
 * `index_path`) is returned instead of throwing.
 */
